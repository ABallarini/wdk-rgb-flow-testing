...
const UNLOCK_CONFIG_BASE = {
  bitcoind_rpc_host: process.env.BITCOIND_RPC_HOST,
  bitcoind_rpc_port: process.env.BITCOIND_RPC_PORT
    ? parseInt(process.env.BITCOIND_RPC_PORT, 10)
    : undefined,
  bitcoind_rpc_username: process.env.BITCOIND_RPC_USERNAME,
...
async function setupAllNodes() {
  ...
  try {
    await axios.post(`${alice.url}/init`, { password: ALICE_PASSWORD });
...
// ============================================================================
// Channel Operations (Initiated via REST from Alice)
// ============================================================================

async function connectPeers() {
  console.log(`\n🔗 Connecting Alice to Bob...`);
  try {
    const peerPubkeyIp = `${NODES.bob.pubkey}@localhost:${NODES.bob.port_ln}`;
    await axios.post(`${NODES.alice.url}/connectpeer`, {
      peer_pubkey_and_addr: peerPubkeyIp,
    });
    ...
    if (
      ...
      await handleError("Alice", "ConnectPeer", error);
    }
  }
}
...
async function openLightningChannel() {
  ...
  try {
    const res = await axios.post(`${NODES.alice.url}/openchannel`, {
      ...
      capacity_sat: BTC_AMOUNT_SAT,
      push_msat: 5000,
      public: true,
      with_anchors: true,
    });
    console.log(`✓ Channel opened. Point: `, res.data);
  } catch (error) {
...
async function waitForChannelActive() {
  ...
  while (!isActive) {
    try {
      ...
      // Find the channel with the specified peer
      const channel = channels.find((c) => c.peer_pubkey === NODES.bob.pubkey);
